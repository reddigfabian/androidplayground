...
@FlowPreview
class LoremPicsumListAdapter(private val lifecycleOwner: LifecycleOwner, viewModelProviderOwner: ViewModelStoreOwner) : LifeCycleItemClickPagingAdapter<LoremPicsumItemViewModel>(ITEM_COMPARATOR) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LifeCycleBindingViewHolder {
        return LifeCycleBindingViewHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_lorem_picsum_list, parent, false))
    }
    ...
    override fun onBindViewHolder(holder: LifeCycleBindingViewHolder, position: Int) {
        getItem(position)?.let { item ->
            (holder.binding as? ItemLoremPicsumListBinding)?.let { itemMainListBinding ->
                ...
                itemMainListBinding.loremPicsumListItemRoot.setOnClickListener { notifyListenersOfClick(item, it) }
                itemMainListBinding.loremPicsumListItemRoot.setOnLongClickListener { notifyListenersOfLongClick(item, it) }
